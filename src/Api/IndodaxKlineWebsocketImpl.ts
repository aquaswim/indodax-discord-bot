...
class IndodaxKlineWebsocketImpl implements IndodaxKlineWebsocket {
    ...
    private _sendUnsubscribeCommand(coidId: string) {
        this.wsMustReady();
        this.logger.info("sending unsubscribe command for", coidId);
    ...
    private _sendSubscribeCommand(coidId: string) {
        this.wsMustReady();
        this.logger.info("sending subscribe command for", coidId);
        this.ws?.send(JSON.stringify({
    ...
    private _connectWs(){
        ...
        this.ws.once("open", () => {
            this.logger.info("Websocket connection to", kLineUrl, "opened");
            this.ready = true;
        ...
        this.ws.on("close", (ws: Websocket, code: number, message: string) => {
            this.logger.error(new Error(`Websocket connection to ${ws.url} closed ${message} ${code}`));
            this.ready = false;
            this._connectWs();
        });
            ...
            if (parsedData.tick) {
                ...
                if (this.subjectInfoDict.hasOwnProperty(tickData.pair)) {
                    ...
                    if (subjectInfo.hasObserver()) {
                        ...
                        this.logger.info("price detected in", tickData.pair, "but no observer found!");
                        subjectInfo.complete();
                        delete this.subjectInfoDict[tickData.pair];
                        this._sendUnsubscribeCommand(tickData.pair);
                    }
                } else {
                    this.logger.warning("Receiving message from", kLineUrl, "that unavailable in subject list");
                    this._sendUnsubscribeCommand(parsedData.tick);
                }
            } else {
                this.logger.info("received non tick data", parsedData);
            }
        })
    }
}
